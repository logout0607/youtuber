...
/**
 * Created by 권 오빈 on 2016. 9. 8..
 */
@Profile("prod")
@Service("imageService")
@Slf4j
public class ProdImageServiceImpl implements ImageService{
	...
	@Value("${fsa.aws.image-bucket-name}") private String bucketName;
	@Value("${fsa.aws.access-key}") private String accessKey;
	@Value("${fsa.aws.secret-access-key}") private String secretAccessKey;
...
//			}
			...
			//String s3Path = storedImageName;
			AmazonS3 s3Client = new AmazonS3Client(new BasicAWSCredentials(accessKey, secretAccessKey));
			try {
				PutObjectResult result = s3Client.putObject(new PutObjectRequest(bucketName, storedImageName, file.getInputStream(), new ObjectMetadata()));
				result = s3Client.putObject(new PutObjectRequest(bucketName, "thumb/" + storedImageName, file.getInputStream(), new ObjectMetadata()));
